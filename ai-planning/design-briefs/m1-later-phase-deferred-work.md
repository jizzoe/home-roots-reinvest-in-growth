...
| --- | --- | --- |
| All receipt behavior: image capture, durable storage, attachment, on-device text recognition (bundled ML Kit), and deterministic parser suggestions | M1.3 Receipt Capture and Extraction Evaluation | Moved out of M1 in full on 2026-09-06. The exit criteria for extraction depend on a frozen corpus of photographed receipts that does not yet exist, and capture was moved with it so M1 could close rather than wait. M1 therefore ships no receipt capability, and PRD REC-001 through REC-004 are recorded as owed at V1. Governed by [M1 Receipt Extraction Acceptance, Evaluation, and Localization Rules](m1-receipt-extraction-acceptance-and-eval-rules.md). |
...
| Haitian Creole human-reviewed UI translations (pass two) | M1.3 Receipt Capture, Extraction Evaluation, and Reviewed Haitian Creole | Pass one shipped as a separately approved component change on 2026-09-05 with every `ht.json` value machine-generated and marked `unreviewed`. Pass two was folded into M1.3 on 2026-09-06 so a single reviewer session can cover both the existing resource set and the new receipt strings that milestone adds. Until it completes, Haitian Creole strings must not be represented as human-reviewed and must not reach participants. |
| Translation-management workflow and production translation tooling | M4 or a later separately approved change | M1.3 delivers one reviewed resource set, not a workflow for managing translations at scale. |
